...
class TestProfileView:
    ...
    def test_profile_view_get(self, client, post_with_group):
        ...
        assert response.status_code != 404, (
            'Страница `/profile/<username>/` не найдена,'
        ...
        assert page_context is not None, (
            'Проверьте, что передали статьи автора'
            + 'в контекст страницы `/profile/<username>/` типа `Page`'
        )
        assert len(page_context.object_list) == 0, (
            'Проверьте, что правильные статьи автора'
            + 'в контекст страницы `/profile/<username>/`'
        )
